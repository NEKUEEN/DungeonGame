    ...
    : m_id(monsterId), m_col(col), m_row(row), m_tileSize(tileSize)
{
    ...
    if (data)
    {
        ...
        // stats + floor scaling (+10% per floor)
    ...
    {
        // fallback ถ้าหา id ไม่เจอ
        std::cerr<<"[Enemy] Monster id not found: "<<monsterId<<"\n";
        m_name="Unknown"; m_family="Unknown";
        m_maxHp=10; m_hp=10; m_attack=3; m_defense=0; m_exp=1;
    }
}
...
void Enemy::loadSprite(const std::string& spriteName)
{
    ...
    else std::cerr<<"[Enemy] Sprite not found: "<<path<<"\n";
}

bool Enemy::updateAI(int pc, int pr, const TileMap& map,
                     const std::vector<Enemy*>& others)
{
    ...
    if (!map.isWalkable(mc,mr))
    { mc=m_col; mr=m_row;
      if (dy!=0) mr+=(dy>0)?1:-1;
      else if (dx!=0) mc+=(dx>0)?1:-1; }

    for (auto* o:others)
        if (o!=this&&o->m_col==mc&&o->m_row==mr) return false;
...
void Enemy::drawHPBar(sf::RenderWindow& window)
{
    ...
    sf::Color c = m_rank==EnemyRank::Boss?sf::Color(255,150,0):
                  m_rank==EnemyRank::Elite?sf::Color(180,80,220):sf::Color(200,60,60);
